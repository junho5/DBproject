...
const main = {
    init: function () {
      const _this = this;
      document.getElementById("submit_total").onclick = function () {
        _this.write();
      };
    ...
    write2: function () {
      ...
      const employee = {
        ...
        project_id: document.getElementById("projectID").innerText,
      ...
      axios.post("/project/modify/search", employee).then(function (result) {
        if (result.data) {
          // 해당 주소에 접속한 결과값이 true이면 /board로 이동
          location.href = "/project/modify/search";
          alert('저장했습니다.')
          console.log('success');
        } else {
          // 만약 false라면 오류입니다 라는 문구를 띄워줌.
    ...
    delete: function () {
      const employee = {
        employee_id : document.getElementById("empID").innerText,
        project_id: document.getElementById("project_id_").value,
      };
      console.log(employee)
      // axios.post("/project/modify/search", employee).then(function (result) {
